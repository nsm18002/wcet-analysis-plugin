...
public class TestingTimeout{
	
	public static AlgCapsule getTestCapsule() {
		AlgCapsule c = new AlgCapsule("TimoutTestCapsule");
		...
		AlgPort timing = new AlgPort("timing", PortKind.TIMING, false);
		...
		AlgSignal timeout = new AlgSignal("timeout", SignalType.IN, timing, timing, timing);
		timing.AddSignal(timeout);
		...
		ArrayList<AlgSignal> timoutList = new ArrayList<AlgSignal>();
		timoutList.add(s2src);
		timoutList.add(s3src);
		sc.AddTWlookupEntry("timing.timeout", new AlgVoPair(200, timoutList));
		...
		transition1.trigger = new AlgTrigger(timing, timeout);
